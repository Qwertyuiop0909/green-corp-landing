...
class Bubble {
    ...
    init() {
        this.color = COLORS[Math.round(4 * Math.random()) % 4];
...
class CanvasBackground {
    ...
    animate() {
        ...
        this.bubbleslist.forEach(a => {
            a.move();
            this.ctx.translate(a.translateX, a.translateY);
            this.ctx.beginPath();
            this.ctx.arc(0, 0, a.size, 0, 2*Math.PI);
            this.ctx.fillStyle = `rgba(${a.color}, ${a.alpha})`;
            this.ctx.fill();
